...
export class OverviewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    interval(40000).pipe(startWith(0), switchMap(() => this.airReadingsService.getLatestReading(this.airUnitId))).subscribe(reading => this.setLatestReadingData(reading));
    interval(40000).pipe(startWith(0),switchMap(() => this.airReadingsService.getGasCAQI(this.airUnitId))).subscribe(caqi => this.evaluateCaqi(caqi));
  }
  ...
  evaluateCaqi(caqiI: number){
    this.caqi = caqiI;
    let cardTitle = document.getElementById("caqi-text");
